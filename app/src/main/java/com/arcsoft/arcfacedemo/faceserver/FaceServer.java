...
public class FaceServer {
    ...
    public FaceEntity registerBgr24(Context context, byte[] bgr24, int width, int height, String name) {
        if (faceEngine == null) {
            Log.e(TAG, "registerBgr24: faceEngine is null, please call FaceServer.getInstance().init() first");
            return null;
        }
        if (context == null) {
            Log.e(TAG, "registerBgr24: context is null");
            return null;
        }
        if (bgr24 == null) {
            Log.e(TAG, "registerBgr24: bgr24 data is null");
            return null;
        }
        if (width % 4 != 0) {
            Log.e(TAG, "registerBgr24: width must be multiple of 4, current width: " + width);
            return null;
        }
        int expectedLength = width * height * 3;
        if (bgr24.length != expectedLength) {
            Log.e(TAG, "registerBgr24: invalid bgr24 data length, expected: " + expectedLength + ", actual: " + bgr24.length);
            return null;
        }
        //人脸检测
        ...
        if (code == ErrorInfo.MOK && !faceInfoList.isEmpty()) {
            ...
            if (code == ErrorInfo.MOK) {
                ...
                if (!maskInfoList.isEmpty()) {
                    ...
                    if (isMask == MaskInfo.UNKNOWN) {
                        Log.e(TAG, "registerBgr24: maskInfo is unknown");
                        return null;
                    }
                }
            } else {
                Log.e(TAG, "registerBgr24 mask process fail res=" + code);
                return null;
            }
            ...
            String userName = name == null || name.trim().isEmpty() ? String.valueOf(System.currentTimeMillis()) : name;
            ...
            if (code == ErrorInfo.MOK) {
                ...
                FaceEntity faceEntity = new FaceEntity(userName, imgPath, faceFeature.getFeatureData());
                long faceId = FaceDatabase.getInstance(context).faceDao().insert(faceEntity);
                faceEntity.setFaceId(faceId);
