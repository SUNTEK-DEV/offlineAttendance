...
public class RecognizeDebugViewModel extends ViewModel implements RecognizeCallback, DebugInfoCallback {
    ...
    public void onAttendanceCheck(CompareResult compareResult) {
        ...
        // Debug模式也真实打卡
        Log.i(TAG, "Debug mode: Processing attendance check for: " + compareResult.getFaceEntity().getUserName());

        // 调用本地考勤集成服务
        com.arcsoft.arcfacedemo.integration.LocalAttendanceIntegration.getInstance(
                com.arcsoft.arcfacedemo.ArcFaceApplication.getApplication())
                .processFaceRecognition(compareResult, new com.arcsoft.arcfacedemo.integration.LocalAttendanceIntegration.ProcessCallback() {
                    ...
                    public void onAttendanceResult(com.arcsoft.arcfacedemo.attendance.model.CheckResult result) {
                        // 打卡成功
                        String resultMsg = "Debug打卡: " + result.getMessage();
                        recognizeNoticeLiveData.postValue(resultMsg);
                        Log.i(TAG, "Debug attendance success: " + resultMsg);
                    }

                    @Override
                    public void onError(String error) {
                        // 打卡失败
                        Log.e(TAG, "Debug attendance error: " + error);
                        recognizeNoticeLiveData.postValue("Debug打卡失败: " + error);
                    }
                });
    }
